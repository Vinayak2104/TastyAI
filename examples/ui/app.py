...
with st.sidebar:
    st.markdown(
        "## How to use\n"
        "1. Just search for the type of dish you need recepie for.\n"
        "2. You can add specific cuisines or specific ingredients.\n"
        "3. You can ask for cooking time.\n"
    )
    st.markdown("---")
    st.markdown("# About")
    st.markdown(
        "TastyAI is an AI-powered recipe app that helps you find and create delicious recipes tailored to your tastes and dietary needs. "
...
rainforest_path = "../data/output.jsonl"
csv_path = "../data/output.jsonl"
...
st.markdown('# 🧑‍🍳 TastyAI - Your Personalized Recipe Assistant')
...
data_sources="RainforestAPI"
question = st.text_input(
    "Search for something",
    placeholder="Which Kind of Food Recepies are you interested in",
    disabled=not "RainforestAPI"
)

# Handle data sources
if DataSource.RAINFOREST_API.value in data_sources:
    shutil.copy("../rainforest/output.jsonl", rainforest_path)
