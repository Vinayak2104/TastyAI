...
def prompt(index, embedded_query, user_query):
    ...
    def build_prompt(local_indexed_data, query):
        ...
        prompt = f"Given the following data of food recepie provide a recepie only if you can or else say i dont know : \n {docs_str} \nanswer this query: {query}, Assume that current date is: {datetime.now()}. and clean the output and try not to give false information provide images if possible"
        return prompt
